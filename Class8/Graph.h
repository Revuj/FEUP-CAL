...
vector<Vertex<T>*> Graph<T>::calculateKruskal() {
    ...
    struct subset *subsets =
            (struct subset*) malloc( vertexSet.size() * sizeof(struct subset) );
    ...
    while (edgeCounter < vertexSet.size() - 1) {
        int u = find(subsets,it->orig->info);
        int v = find(subsets,it->dest->info);
